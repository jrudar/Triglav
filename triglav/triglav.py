...
class ETCProx:
    def __init__(
        self, n_estimators=1024, min_samples_split=0.33, n_sets=5, return_clf=False
    ):
...
def f_stat(X, y):
    ...
    SSW = permanova_f_stat_sW_cy(DistanceMatrix(D).data, group_sizes, grouping)

    SSA = SST - SSW

    return (SSW / (X.shape[0] - n_groups)) / (
        SSA / (n_groups - 1)
...
class DSFSProblem(Problem):
    ...
    def __init__(
        self, lb=None, ub=None, minmax="min", X=None, y=None, M=None, **kwargs
    ):
    ...
    def __set_domain_range(self, lb, ub):
        if type(lb) in self.SUPPORTED_ARRAY and type(ub) in self.SUPPORTED_ARRAY:
            ...
            if len(self.lb) == len(self.ub):
                ...
                if len(self.lb) < 1:
                    raise ValueError(
                        f"Dimensions do not qualify. Length(lb) = {len(self.lb)} < 1."
                    )
            else:
                raise ValueError(
                    f"Length of lb and ub do not match. {len(self.lb)} != {len(self.ub)}."
                )
        else:
    ...
    def __set_functions(self, kwargs):
        ...
        if "amend_position" in kwargs:
            if not callable(self.amend_position):
                raise ValueError(
                    f"Use default 'amend_position()' or passed a callable function. {type(self.amend_position)} != function"
                )
            else:
            ...
            if self.n_objs > 1:
                ...
                if type(self.obj_weights) in self.SUPPORTED_ARRAY:
                    ...
                    if self.n_objs != len(self.obj_weights):
                        raise ValueError(
                            f"{self.n_objs}-objective problem, but N weights = {len(self.obj_weights)}."
                        )
                    self.msg = f"Solving {self.n_objs}-objective optimization problem with weights: {self.obj_weights}."
                else:
                    raise ValueError(
                        f"Solving {self.n_objs}-objective optimization, need to set obj_weights list with length: {self.n_objs}"
                    )
            elif self.n_objs == 1:
                ...
                raise ValueError(
                    f"fit_func needs to return a single value or a list of values list"
                )
        elif (
            type(result) in (int, float)
            or isinstance(result, np.floating)
            or isinstance(result, np.integer)
        ):
            self.multi_objs = False
    ...
    def fit_func(self, x):
        ...
        log_losses_1 = [
            log_loss(
                self.y[test_ind[i]],
                c1["estimator"][i].predict_proba(x_train[test_ind[i]]),
            )
            for i in range(3)
        ]
...
def select_features(
    ...
    if (run_stage_2 == "mms") or (run_stage_2 == "auto") or (run_stage_2 == True):
        if (run_stage_2 == "mms") or (run_stage_2 == True):
            if verbose > 0:
                print(
                    "Stage Two: Identifying best features using a modified MultiSURF..."
                )

            F_selector = stage_2_mms(
                transformer.fit_transform(X_red[:, S]), y, stage_2_estimator
            )
            ...
            if verbose > 0:
                print(
                    "Stage Two: Identifying best features using swarm optimization..."
                )

            S2_features, F_selector = stage_2_soa(
                transformer.fit_transform(X_red[:, S]), y, stage_2_estimator
            )
...
def stage_2_mms(
    X: np.ndarray, y: np.ndarray, M: Type[ClassifierMixin, BaseEstimator]
) -> MultiSURF:
    """
    ...
    for n_selected in [
        ...
        if n_selected < X.shape[1]:
            ...
            score = cross_val_score(
                estimator=M, X=X_trf, y=y, scoring="balanced_accuracy", cv=5, n_jobs=5
            ).mean()
...
def stage_2_soa(
    ...
    FS_problem = DSFSProblem(
        lb=np.zeros(shape=(X.shape[1]), dtype=int),
...
class Triglav(TransformerMixin, BaseEstimator):
    ...
    def __init__(
        ...
        transformer=NoScale(),
        sampler=NoResample(),
        estimator: Type[ClassifierMixin, BaseEstimator] = ExtraTreesClassifier(
            512, bootstrap=True
        ),
        stage_2_estimator: Type[ClassifierMixin, BaseEstimator] = ExtraTreesClassifier(
            512
        ),
    ...
    def _check_params(
        ...
        if isinstance(self.alpha, float):
            if self.alpha <= 0 or self.alpha > 1:
                raise ValueError(
                    "The 'alpha' parameter should be a float between 0 and 1."
                )
        else:
        ...
        if isinstance(self.p_1, float):
            if (self.p_1 <= 0) or (self.p_1 > 1):
                raise ValueError(
                    "The 'p_1' parameter should be a float between 0 and 1."
                )
        else:
        ...
        if isinstance(self.p_2, float):
            if (self.p_2 <= 0) or (self.p_2 > 1):
                raise ValueError(
                    "The 'p_2' parameter should be a float between 0 and 1."
                )
        else:
        ...
        if isinstance(self.verbose, int):
            ...
            raise TypeError(
                "The 'verbose' parameter should be an integer greater than \
            or equal to 0."
            )
        ...
        if isinstance(self.n_iter, int):
            ...
            raise TypeError(
                "The 'n_iter' parameter should be an integer greater than \
            or equal to 0."
            )
        ...
        if isinstance(self.n_iter_fwer, int):
            ...
            raise TypeError(
                "The 'n_iter_fwer' parameter should be an integer greater than \
            or equal to 0."
            )
        ...
        if isinstance(self.n_jobs, int):
            ...
            raise TypeError(
                "The 'n_jobs' parameter should be an integer greater than \
            or equal to 1."
            )
        ...
        if isinstance(self.thresh, int) or isinstance(self.thresh, float):
            ...
            raise TypeError(
                "The 'thresh' parameter should be an integer or float \
        ...
        if self.run_stage_2 not in stage_2_params:
            raise ValueError(
                "The 'run_stage_2' parameter should be a string set to 'auto' or 'mms' \
