...
def getVarianceofStability(Z):
    ...
    hatPF = np.mean(
        Z, axis=0
    )  # hatPF is a numpy.array with the frequency of selection of each feature
    kbar = np.sum(
        hatPF
    )  # kbar is the average number of selected features over the M feature sets
    k = np.sum(
        Z, axis=1
    )  # k is a numpy.array with the number of features selected on each one of the M feature sets
    denom = (kbar / d) * (1 - kbar / d)
    stab = (
        1 - (M / (M - 1)) * np.mean(np.multiply(hatPF, 1 - hatPF)) / denom
    )  # the stability estimate
    phi = np.zeros(M)
    for i in range(M):
        phi[i] = (1 / denom) * (
            np.mean(np.multiply(Z[i,], hatPF))
            - (k[i] * kbar) / d**2
...
def confidenceIntervals(Z, alpha=0.05, res={}):
    ...
    if len(res) == 0:
        res = getVarianceofStability(
            Z
        )  # get a dictionnary with the stability estimate and its variance
    lower = res["stability"] - norm.ppf(1 - alpha / 2) * math.sqrt(
        res["variance"]
    )  # lower bound of the confidence interval at a level alpha
